...
function ShowAdaptation(plnat,tnat){
	...
	if(String(localStorage.mycountry)!='undefined' && plnat!=undefined && plnat!=' '){
		$.getScript("js/adaptation.en.js", function() {
			...
			txt+='<tr align=left><th>'+localStorage.mycountry.split('.')[1]+'</th><td>'+(ad==10 ? '100' : (ad*6)+'%-'+(ad*6+40))+'% ('+ad+')</td></tr>'
			if(tnat!=undefined && tnat!=parseInt(localStorage.mycountry)){
				...
				txt+='<tr align=left><th>'+natname+'</th><td>'+(tad==10 ? '100' : (tad*6)+'%-'+(tad*6+40))+'% ('+tad+')</td></tr>'
			}
			txt+='</table>'
