...
$().ready(function() {
	...
	$('td.back4 table tr:gt(2) td[width=128]').each(function(){
		...
		// span.text2b a(им€) img(человечек) img($, может не быть)
		if(forumflags[1]==1) $(this).find('span.text2b img').remove()

		// a(клуб)
		if(forumflags[2]==1) $(this).find('a[href^="plug.php?p=refl&t=k&j="]').remove()

		// span.text1s a(ѕользователь) br
		if(forumflags[3]==1) $(this).find('span.text1s:has(a:contains(ѕользователь))').remove()
...
//		if(forumflags[7]==1)
		...
		// " "
		// span.text1xs "откуда" img(flag) <br> професси€ <hr> img a(сборна€) <hr> регалии
	...
	// пожаловатьс€ модератору
	if(forumflags[13]==1) $('td.back4 table tr:gt(2) td.back2:contains(ѕожаловатьс€ модераторам)').each(function(){$(this).html($(this).html().split('<br>')[0])});
