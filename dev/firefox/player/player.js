...
//		if ($(val).html().replace(/<!-- [а-я] -->/g,'').indexOf(str) != -1) um = i
		...
		if ($(val).html().replace(/<!-- [а-я] -->/g,'').indexOf(str3) != -1) fr = i
	})
			...
			$(val).find('center').each(function(m,valm){
				if (m==0) {
					...
					if (name.indexOf(' ')!=-1){
						...
						player[st['Фам']] = name.replace(player[st['Имя']]+' ','')
					} else {
						player[st['Имя']] = ''
						player[st['Фам']] = name
					}	
					...
					player[st['иМл']] = (x[j].indexOf('U21')!=-1 ? +x[j].split('/ U21 матчей ',2)[1].split(',',1)[0] : 0)
					player[st['гМл']] = (x[j].indexOf('U21')!=-1 ? +x[j].split('/ U21 матчей ',2)[1].split(', голов ',2)[1].replace(')','') : 0); j++
